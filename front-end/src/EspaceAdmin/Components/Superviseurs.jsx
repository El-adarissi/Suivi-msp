...
const EMPTY_FORM = { NomSup: "", PrenomSup: "", Email: "", Password: "", is_active: 1 };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 w-full max-w-md shadow-xl">
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          ...
          {[
            { field: "Email",    label: "Email",        type: "email",    placeholder: "karim@email.com" },
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="is_active" className="text-sm text-slate-700 dark:text-slate-300">Compte actif</label>
          </div>
...
export function Superviseurs() {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) { fetchSuperviseurs(); setModal(null); }
  ...
  const handleEdit = async (e) => {
    ...
    setSaving(true);
    try {
      const res  = await fetch(`${API_URL}/api/superviseur/${selected.Id_Superviseur}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(form),
      });
  ...
  const handleDelete = async (id) => {
    try {
      const res  = await fetch(`${API_URL}/api/superviseur/${id}`, { method: "DELETE" });
      const json = await res.json();
      if (json.success) { fetchSuperviseurs(); setDeleteId(null); }
    } catch (err) { console.error(err); }
  };
  ...
  return (
    <div className="space-y-6">
      {modal === "add" && (
        <ModalForm title="Nouveau superviseur" form={form} setForm={setForm}
          onSubmit={handleAdd} onClose={() => setModal(null)} saving={saving} />
      )}
      {modal === "edit" && (
        <ModalForm title="Modifier le superviseur" form={form} setForm={setForm}
          onSubmit={handleEdit} onClose={() => setModal(null)} saving={saving} />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 w-full max-w-sm p-6 shadow-xl space-y-4">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Cette action est irréversible. Les stagiaires associés seront désaffectés.
            </p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Gestion des collaborateurs responsables de l'encadrement.</p>
        </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-10 text-center text-slate-400">Aucun superviseur trouvé.</td>
                </tr>
              ) : data.map((item) => (
                <tr key={item.Id_Superviseur} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium text-slate-900 dark:text-white">{item.NomSup} {item.PrenomSup}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400">
                      {item.NbStagiaires} stagiaire{item.NbStagiaires !== 1 ? "s" : ""}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={() => setDeleteId(item.Id_Superviseur)} title="Supprimer"
