...
import { User, Mail, Save, ShieldCheck, Loader2, CheckCircle, AlertCircle, BadgeCheck } from "lucide-react";
...
export function ProfileSup() {
  ...
  // Load Profile using exact schema columns
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/superviseur/profile`, {
          headers: authHeaders()
        });
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await axios.put(`${API_URL}/api/superviseur/profile/update`, {
        NomSup: formData.NomSup,
        PrenomSup: formData.PrenomSup,
        Email: formData.Email
      }, {
        headers: authHeaders()
      });
      notify("Informations enregistrées avec succès !");
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-500 space-y-3">
        ...
        <p className="text-sm font-medium">Chargement du profil superviseur...</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start animate-fade-in text-slate-900 dark:text-slate-100 p-1">
      ...
      <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
        <h2 className="text-xl font-bold text-slate-950 dark:text-white mb-1">Mon Profil Encadrant</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Gérez vos informations de compte personnelles et administratives.</p>
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-2">Adresse Email Professionnelle</label>
      ...
      <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-6 shadow-sm">
        <div className="flex flex-col items-center text-center pb-4 border-b border-slate-200 dark:border-slate-800">
          ...
          <h3 className="font-bold text-lg text-slate-900 dark:text-white">
            {formData.PrenomSup} {formData.NomSup || "Utilisateur"}
          </h3>
        ...
        <div className="space-y-4 text-sm">
          <div className="flex items-center gap-3 text-slate-600 dark:text-slate-300">
            ...
            <div>
              <p className="text-xs text-slate-400">Rôle Système</p>
              <p className="font-medium text-sm capitalize">{formData.role}</p>
            </div>
          </div>
